...
class _PackedLoRAAWeightsMixin(MergedColumnParallelLinearWithLoRA):
    def create_lora_weights(
        ...
        # Block-diagonal packed B: [n_slices * rank, sum(output_slices)] so the
        # whole expand step becomes a single GEMM. Zero blocks keep slices
        # decoupled exactly like the per-slice computation.
    ...
    def _apply_lora_to_output(self, x: torch.Tensor, output: torch.Tensor) -> torch.Tensor:
        ...
        lora_output: torch.Tensor | None = self.punica_wrapper.add_lora_linear(
            ...
            packed_lora_b=self.lora_b_packed,
        )
